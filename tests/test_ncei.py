...
def _find_test_token() -> str:
    """
    In the case of parallelizing unit tests, we will allow the user to
    set the token via an environment variable `NCEI_TEST_TOKEN`
    """
    return environ.get("NCEI_TEST_TOKEN", get_default_token())
...
def test_query_good_token():
    ...
    """
    results = query_all(_find_test_token(), obj_type=DataType)
...
def test_query_obj_type_bad_base():
    ...
    """
    results = query_all(_find_test_token())
    assert len(results) == 0
...
def test_query_obj_type_bad_type():
    ...
    with pytest.raises(AttributeError):
        results = query_all(_find_test_token(), obj_type=ExampleTest)
...
def test_query_base_good():
    ...
    """
    results = query_base(_find_test_token(), DataType.endpoint, obj_type=DataType)
...
def test_query_base_bad():
    ...
    """
    results = query_base(_find_test_token(), NCEIBase.endpoint+"bad_data", obj_type=DataType)
    assert len(results) == 0
...
def test_query_limit_more_than_one():
    ...
    """
    results = query_base(_find_test_token(), DataType.endpoint, obj_type=DataType, limit=1000, offset=1)
...
def test_query_offset_different():
    ...
    """
    results_one = query_base(_find_test_token(), DataType.endpoint, obj_type=DataType)
    results_two = query_base(_find_test_token(), DataType.endpoint, obj_type=DataType, offset=2)
